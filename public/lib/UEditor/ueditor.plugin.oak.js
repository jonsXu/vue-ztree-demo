...
// var emreCmds = [
//   'template','controllibrary','checkbox','radio','select','input','timeinput','textarea'
// ];暂时放弃timeInput
var emreCmds = [
  'template','controllibrary','checkbox','radio','select','input','textarea',
];
//页面布局
var layoutCmds = [
...
function command(editor){
  ...
  //对基础固定工具栏进行重写,允许自己自定义传值
  var fixedH = {}
  if(editor.options.tool_bars!==undefined&&editor.options.tool_bars.fixedHtml !=undefined){
    fixedH = new OAKEditor({editor:editor,command:editor.options.tool_bars.fixedHtml});
  } else {
    fixedH = new OAKEditor({editor:editor,command:fixedCmds});
  }
